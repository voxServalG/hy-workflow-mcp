...
10. hy_reset — 重置工作流回到 init 阶段。任何阶段卡住时可用。
...
调用 hy_plan({task: "描述你要做的任务"})。服务端自动分析项目上下文（garden-scan）并用 DeepSeek API 生成 PlanDoc。PlanDoc 生成后必须经过 7 道验证关才会被接受。
...
状态机卡住:   调用 hy_reset 回到 init 重新开始。
