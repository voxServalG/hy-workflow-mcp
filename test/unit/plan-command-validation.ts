...
async function expectRejected(label: string, mutate: (plan: PlanDoc) => void): Promise<void> {
  ...
  if (!(result.error?.message ?? String(result.error)).includes("pure executable shell command")) {
    throw new Error(`${label} returned an unclear error: ${result.error?.message ?? JSON.stringify(result.error)}`);
  }
}
