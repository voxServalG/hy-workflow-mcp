...
export function setupUpdateRequiredResult(check) {
...
function readCurrentPhaseReadonly() {
    try {
...
export function attachSetupCheck(result, check) {
    return {
        ...result,
        setupUpdateCheck: check,
    };
}
export function createSetupGate(root = projectRoot()) {
    let checked = false;
    return () => {
        if (checked)
            return null;
