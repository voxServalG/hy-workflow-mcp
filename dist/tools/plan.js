...
function pathReason(path, action) {
    if (path.startsWith("src/"))
        return action === "delete" ? "移除不再需要的源码入口" : "调整运行时代码行为";
    if (path.startsWith("dist/"))
        return action === "delete" ? "移除对应编译产物" : "同步发布用编译产物";
    if (path.startsWith("tests/"))
        return action === "delete" ? "移除不再适用的测试" : "更新验证覆盖";
    if (path.startsWith("docs/"))
        return action === "delete" ? "移除过期文档" : "同步用户和 agent 可读说明";
    if (path === "README.md")
        return action === "delete" ? "移除入口说明" : "同步项目入口说明";
    if (action === "new")
        return "新增本次任务需要的文件";
    if (action === "delete")
        return "删除本次任务不再需要的文件";
    return "更新本次任务涉及的文件";
}
function addPathList(lines, paths, action) {
    if (!paths.length) {
        lines.push("- 无");
        return;
    }
...
function classifyChecks(smoke, tests) {
    const categories = [
        {
    ...
    const assign = (check, fallback) => {
        const text = `${check.command} ${check.description}`.toLowerCase();
        if (/acceptance|验收|user|用户/.test(text))
            categories[3].checks.push(check);
        else if (/integration|集成|e2e|端到端/.test(text))
            categories[1].checks.push(check);
        else if (/system|系统|dist|doclint|codelint|build|compile|tsc|lint/.test(text))
            categories[2].checks.push(check);
        else if (/unit|单元|vitest|jest|test|tsx/.test(text))
            categories[0].checks.push(check);
        else
            categories[fallback].checks.push(check);
    };
    smoke.forEach(check => assign(check, 2));
    tests.forEach(check => assign(check, 0));
    return categories;
}
function addCheck(lines, check) {
...
function buildSummary(p) {
    ...
    if (plat.setup.length)
        plat.setup.forEach(command => lines.push(`- command: \`${command}\``));
    else
        lines.push("- command: 无需额外搭建命令");
    for (const category of classifyChecks(p.verify.smoke, p.verify.tests)) {
        ...
        if (category.checks.length)
            category.checks.forEach(check => addCheck(lines, check));
        else
            lines.push("- 本次计划未声明这一层级的测试。");
    }
    lines.push("");
    lines.push("### Risks");
