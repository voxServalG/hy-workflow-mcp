...
export async function handleBranch(args) {
    const state = readState();
    assertPhase(state, "approve", "branch");
    if (!state.plan)
        return toolResult("branch", { error: "No plan found. Run hy_plan first.", allowedTools: ["hy_status"] });
    ...
    if (!result.ok)
        return toolResult("branch", { error: result.error, allowedTools: ["hy_branch", "hy_status"] });
