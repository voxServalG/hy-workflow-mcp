...
export async function handleMerge() {
    ...
    if (!state.prNumber)
        return toolResult("merge", { error: "No active PR", allowedTools: ["hy_status"] });
    const result = mergePr(state.prNumber);
    if (!result.ok)
        return toolResult("merge", { error: result.error, requires_user: true, stop_here: true, recovery: { tool: "hy_merge", instruction: "Inspect the merge failure, resolve blockers, then retry hy_merge if the approved workflow is still valid." }, allowedTools: ["hy_merge", "hy_status"] });
