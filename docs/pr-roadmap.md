...
| PR-02 Make setup-to-first-use human-readable | [#121](https://github.com/voxServalG/hy-workflow-mcp/issues/121), reference [#133](https://github.com/voxServalG/hy-workflow-mcp/issues/133) | Rewrite setup output so the copied prompt asks the agent to confirm detected `baseBranch`, `docsDir`, `codeDirs`, `codeExt`, tracked artifacts, and local artifacts in user language. | New users know what to paste and what response to expect; ambiguous config asks concrete questions; artifact boundaries stay unchanged. | `npm run build`; `npm run test:setup-update`; `npm run test:e2e` |
...
| PR-11 Rewrite README first screen and add recipes | [#121](https://github.com/voxServalG/hy-workflow-mcp/issues/121), [#133](https://github.com/voxServalG/hy-workflow-mcp/issues/133), references [#131](https://github.com/voxServalG/hy-workflow-mcp/issues/131), [#132](https://github.com/voxServalG/hy-workflow-mcp/issues/132) | Rewrite README around the safety-belt value proposition; add minimal install, first prompt, and recipes for first-run, docs-only, code-change, verify-fail, CI-red, setup-artifact-sync. | A new user can install, restart client, and call `hy_status` without reading all docs; README explains commit vs local artifacts; recipes are short and linked. | `npm run build`; `npm run lint:contract`; `npm test` |
