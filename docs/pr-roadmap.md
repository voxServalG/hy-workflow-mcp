...
| PR-02 Make setup-to-first-use trustworthy | [#121](https://github.com/voxServalG/hy-workflow-mcp/issues/121), reference [#133](https://github.com/voxServalG/hy-workflow-mcp/issues/133) | One Node TUI/CLI validates effective Codex/Claude/OpenCode user scopes, docs/base readiness and transactional external state; fresh setup creates exactly two artifacts without review, while occupied targets remain unread unless an explicit exact artifact sync is requested. | Success means direct-bin handshakes, selected authority, clients, registry and ownership agree; cancel/dry-run write nothing; concurrent/fault recovery is fail closed. | `npm run verify`; `npm run test:acceptance`; `npm pack --dry-run --json` |
...
| PR-05 Harden GitHub API retry and fallback | [#37](https://github.com/voxServalG/hy-workflow-mcp/issues/37) | Separate immutable PR identity from mutable lifecycle; persist attempted/confirmed/sync receipt stages; serialize merge operations with a project lock; keep `executePrMerge` as the only mutation; reconcile GitHub postcondition before fresh-fetch ancestry through a read-only Git fallback; stage downstream rebases detached before compare-and-swap/leased installation. | Exact repository/base/head/verified-OID evidence is required; one receipt invokes merge mutation at most once; real stacked candidates require verified-head ancestry, and normal pre-mutation snapshots also require prepared-base ancestry plus local=remote; confirmed integration pins `syncBaseOid` and resumes persisted `rebasing`/`resultOid` progress; local CAS, force-with-lease, and base ancestry drift checks fail closed; legacy no-receipt recovery syncs only provable stacks and refuses diverged refs; success reports outcome and executor. | `npm run build`; `npm run test:unit`; `npm run test:e2e`; `npm run lint:contract` |
