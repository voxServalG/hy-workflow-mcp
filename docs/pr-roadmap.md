...
| PR-02 Make setup-to-first-use human-readable | [#121](https://github.com/voxServalG/hy-workflow-mcp/issues/121), reference [#133](https://github.com/voxServalG/hy-workflow-mcp/issues/133) | Provide one Node TUI that detects Codex, Claude Code, and OpenCode; includes update and unset; always maintains only `hy-workflow.json` and the GitHub workflow while keeping runtime/client data external. | New users can select clients and finish setup without copying a prompt; the two team files are explicit; automation has deterministic flags and no mode branch. | `npm run build`; `npm run test:setup-update`; `npm run test:e2e` |
