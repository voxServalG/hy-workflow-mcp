...
| PR-02 Make setup-to-first-use human-readable | [#121](https://github.com/voxServalG/hy-workflow-mcp/issues/121), reference [#133](https://github.com/voxServalG/hy-workflow-mcp/issues/133) | Provide one Node TUI that detects Codex, Claude Code, and OpenCode; defaults config/state/cache to OS user storage; includes update and unset; keeps shared artifacts opt-in. | New users can select clients and finish setup without copying a prompt; local mode has zero project diff; automation has explicit flags. | `npm run build`; `npm run test:setup-update`; `npm run test:e2e` |
...
| PR-11 Rewrite README first screen and add recipes | [#121](https://github.com/voxServalG/hy-workflow-mcp/issues/121), [#133](https://github.com/voxServalG/hy-workflow-mcp/issues/133), references [#131](https://github.com/voxServalG/hy-workflow-mcp/issues/131), [#132](https://github.com/voxServalG/hy-workflow-mcp/issues/132) | Rewrite README around the safety-belt value proposition; add npm install, setup/unset TUI, local/shared modes, and recovery recipes. | A new user can install, select clients, restart, and call `hy_status`; README makes zero-change default and shared artifacts explicit. | `npm run build`; `npm run lint:contract`; `npm test` |
