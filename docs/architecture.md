...
Helper has a short-lived interactive terminal UI for presentation and input; the same lifecycle services back its JSON commands. It writes only user-level Skill data, ownership state, and detected Agent Skill directories. It works outside a Git repository. It never writes project files, `.git`, Agent project configuration, MCP configuration, or GitHub Actions. A missing projection of a current packaged Skill is drift and ordinary update restores it. A 0.5 ownership manifest is migrated transactionally from 12 Skills to 3; an owned resource whose type, path, target, or hash changed is preserved and reported as a conflict rather than guessed away.
