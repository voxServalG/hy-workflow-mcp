...
- setup: the TUI/CLI, doctor, schema-3 deployment, transaction journal, adapters, template and checked-in workflow must preserve one deployment model, direct-bin effective client definitions, explicit CI/artifact acceptance and exactly three team-owned repository surfaces (`hy-workflow.json`, `.github/workflows/hy-workflow.yml`, and the managed `<!-- hy-workflow-rules -->` block in `AGENTS.md`, with content outside the markers preserved byte-for-byte). Runtime reads only root hy-workflow.json, verifies tool/catalog and artifact evidence, treats legacy config/stamps/compatibility JSON as read-only migration inputs, and embeds the deterministic first-party lint bundle into the pull-request/manual workflow without downloads or project-file mutation.
