...
- setup: the TUI/CLI, doctor, schema-3 deployment, transaction journal, adapters, template and checked-in workflow must preserve one deployment model, direct-bin effective client definitions, explicit CI/artifact acceptance and exactly two team files. Runtime reads only root hy-workflow.json, verifies tool/catalog and artifact evidence, and treats legacy config/stamps as migration inputs. Compatibility JSON remains temporary and restoration failures remain hard.
