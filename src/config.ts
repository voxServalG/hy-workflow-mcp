...
function codelintConfig(existing: JsonObject | null, suggestion: ConfigSuggestion, preserveExisting: boolean): JsonObject {
  if (preserveExisting) {
    return {
...
function doclintConfig(existing: JsonObject | null, suggestion: ConfigSuggestion, preserveExisting: boolean): JsonObject {
  if (preserveExisting) {
    return {
...
function gardenerConfig(existing: JsonObject | null, suggestion: ConfigSuggestion, preserveExisting: boolean): JsonObject {
  if (preserveExisting) {
    return {
      docsDir: existing?.docsDir ?? suggestion.docsDir,
      codeDirs: existing?.codeDirs ?? suggestion.codeDirs,
...
export function checkConfig(root: string, suggestion = defaultSuggestion(root)): ConfigCheckResult {
  ...
  if (!codelint) issues.push("Missing codelint.json");
  if (!doclint) issues.push("Missing doclint.json");
  if (!gardener) issues.push("Missing docs-gardener.json");
  const expectedExt = project.kind === "python" ? ".py" : project.kind === "typescript" ? ".ts" : null;
  if (expectedExt && codelint?.codeExt && codelint.codeExt !== expectedExt) issues.push(`codelint.json codeExt=${codelint.codeExt} but project appears ${project.kind}`);
  for (const [file, config] of Object.entries({ "doclint.json": doclint, "docs-gardener.json": gardener })) {
    if (!config) continue;
    if (expectedExt && config.codeExt && config.codeExt !== expectedExt) issues.push(`${file} codeExt=${config.codeExt} but project appears ${project.kind}`);
    if (config.docsDir && !exists(root, config.docsDir)) issues.push(`${file} docsDir does not exist: ${config.docsDir}`);
    for (const dir of valueArray(config.codeDirs)) {
      if (!exists(root, dir)) issues.push(`${file} codeDirs entry does not exist: ${dir}`);
    }
  }
  if (doclint && gardener && JSON.stringify(doclint.codeDirs) !== JSON.stringify(gardener.codeDirs)) issues.push("doclint.json codeDirs differs from docs-gardener.json codeDirs");
  if (doclint && gardener && doclint.docsDir !== gardener.docsDir) issues.push("doclint.json docsDir differs from docs-gardener.json docsDir");
...
function parseArgs(argv: string[]): ConfigArgs {
  const args: ConfigArgs = { mode: "check", json: false, dryRun: false, applySuggested: false, explicit: {} };
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (arg === "--base-branch") args.explicit.baseBranch = next();
  }
