...
export function setupUpdateRequiredResult(check: SetupCheck): ToolResult {
  ...
  const reason = check.status === "artifact_drift"
    ? `The three team artifacts, including any embedded built-in lint bundle, no longer match the deployment evidence: ${check.artifactDrift?.map(item => `${item.file} (${item.reason})`).join(", ") ?? "unknown drift"}.`
