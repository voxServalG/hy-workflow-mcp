...
export function normalizeCodeExt(value: unknown): string[] {
...
export function codeExtOr(value: unknown, fallback: CodeExt): CodeExt {
  if (typeof value === "string") return value;
  if (Array.isArray(value) && value.every(item => typeof item === "string")) return value;
  return fallback;
}

export function formatCodeExt(value: CodeExt): string {
  return normalizeCodeExt(value).join(",");
}

export function validateCodeExt(value: unknown): string[] {
  const exts = normalizeCodeExt(value);
  if (!exts.length) return ["project.codeExt must include at least one extension"];
  return exts
    .filter(ext => !/^\.[A-Za-z0-9][A-Za-z0-9+_-]*$/.test(ext))
    .map(ext => `project.codeExt entry is invalid: ${ext}`);
}

export function isKnownCodeExt(ext: string): boolean {
